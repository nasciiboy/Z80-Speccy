...
OpenGray:
 ld hl,$CE00                         ; Save six bytes after _plotScreen
 ld de, SixBytes                     ;  since our int routine overwrites it
 ld bc, 6
 ldir
 ...
 ld hl,int_copy                      ; Source = grayscale interrupt routine)
...
;===============================================================================
; Set up parameters to pass the interrupt handler via the alternate register set
;===============================================================================
 di
 exx                                ; Exchange regular registers with alternate registers
 ld b,$3c                           ; $3C used in int routine for Port 0 ($FC00)
 ld c,0                             ; Used because variables get cleared often in the int routine
 ld d,5                             ; 5 is the user counter
 ld e,%110110                       ; This is used to XOR $3C to get $0A and vise versa
 ld hl,UserCounter                  ; HL points to the counter, needed in the int routine
 exx                                ; Normal registers restored; alternate registers contain parameters
...
CloseGray:
 ...
 ld de, $CE00                         ;  since our int routine overwrites it
 ld bc, 6
 ldir
...
int_copy:
.org $8f8f                             ; Interrupt routine starts here

int_start:
 ex af,af'                             ; Exchange registers. 
 exx                                   ; Parameters were passed via alternate register set

 in a,(3)                              ; Is the LCD in the middle of refreshing?
 bit 1,a
 jr z,leave_int                        ; If so, immediately exit interrupt routine

 inc (hl)                              ; UserCounter = UserCounter + 1
 out (c),b                             ; Output $3C or $0A (see change_pages) to Port $0 
                                       ;  This forms the addresses $FC00 or $CA00
 dec d                                 ; Subtract from grayscale counter
 call z,reset_int_counter              ; If the grayscale falls to 0, then reset the counter
 ld a,d
...
leave_int:
 ...
 rra                                   ; Check bit 0 (ON interrupt status)
 ld a,c                                ; A = 0 
 adc a,9                               ; A = 9 + Carry Flag (either $09 or $0A)
 out (3),a                             ; Send $09 if ON not pressed, $0A otherwise
 ld a,$0B
...
change_pages:
 ld a,e                                ; E = %110110 
 xor b                                 ; XOR $3C with E to get $0A, and XOR $0A with E to get $3C
                                       ; Thus the B register alternates between $FC and $CA

 ld b,a                                ; B register saved for later use
 ret                                   ; Return
int_end:
